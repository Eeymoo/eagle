...
export type JellyfinVideoChannel = Channel & { source: 'jellyfin-video' };
...
export class JellyfinVideoSource extends LiveSourceBase {
  ...
  private cache?: JellyfinVideoChannel[];
  /** itemId → item, for stream resolution metadata. */
  private byId = new Map<string, ItemDto>();
  ...
  async listChannels(opts?: ListChannelsOpts): Promise<ChannelPage> {
    if (!opts?.force && this.cache) return { channels: this.cache, nextCursor: undefined };

    const channels: JellyfinVideoChannel[] = [];
    ...
    for (let start = 0; start < MAX_ITEMS; start += PAGE_SIZE) {
      ...
      if (items.length < PAGE_SIZE || channels.length >= (dto.TotalRecordCount ?? 0)) break;
    }
    this.cache = channels;
    this.byId = byId;
  ...
  private toChannel(item: ItemDto): JellyfinVideoChannel {
 ...
 */
export const jellyfinVideoPlugin: SourcePlugin = {
  kind: 'jellyfin-video',
  ...
  async connect(port, input) {
    const { label, ...config } = input as unknown as JellyfinVideoInput;
  ...
  create(port: Port, connection: PluginConnection) {
