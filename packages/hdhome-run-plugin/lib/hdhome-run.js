 ...
 */
export const hdHomeRunPlugin = {
    kind: 'hdhome-run',
    displayName: 'HDHomeRun',
    channelIdPrefix: 'hdhr',
    async connect(port, input) {
        const { deviceUrl, label } = input;
        if (!deviceUrl)
            throw new CoreError('PARSE', 'HDHomeRun: deviceUrl is required');
    ...
    create(port, connection) {
        const { device } = connection.state;
        return new HDHomeRunSource(port, device, connection.id);
    },
};
//# sourceMappingURL=hdhome-run.js.map